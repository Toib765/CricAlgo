...
def best_partnerships():
    rows = query("""
        SELECT pt.*, m.match_date, m.match_stage,
               ...
               pt.batting_team AS team
        FROM PARTNERSHIP pt
        JOIN PLAYER p1 ON p1.player_id=pt.player1_id
        JOIN PLAYER p2 ON p2.player_id=pt.player2_id
        LEFT JOIN MATCHES m ON m.match_id=pt.match_id
...
def fantasy_suggest():
    ...
    import random
    for p in available_bat + available_bowl + available_wkar:
        p['value'] = round(p['value'] + random.uniform(-2.0, 2.0), 2)
    ...
    return jsonify({
        ...
        "updated_pool": available_bat + available_bowl + available_wkar
    })
...
@app.route('/api/daa/sort_race')
def daa_sort_race():
    players_db = query("SELECT p.*, t.country_name FROM PLAYER p JOIN TEAM t ON t.team_id=p.team_id")
    if not players_db: return jsonify({})
    
    players_list = [dict(p) for p in players_db]
    
    # Quick Sort
    start_q = time.perf_counter()
    q_sorted = daa_algos.quick_sort_players(list(players_list), key='no_of_totalruns', reverse=True)
    time_q = (time.perf_counter() - start_q) * 1000 # ms
    
    # Merge Sort
    start_m = time.perf_counter()
    m_sorted = daa_algos.merge_sort_impact(list(players_list), key='no_of_totalruns', reverse=True)
    time_m = (time.perf_counter() - start_m) * 1000 # ms
    
    return jsonify({
        "quick_sort_ms": round(time_q, 4),
        "merge_sort_ms": round(time_m, 4),
        "top_10": q_sorted[:10]
    })

@app.route('/api/daa/convex_hull')
def daa_convex_hull():
    batters = query("""
        SELECT p.player_name, p.strike_rate as sr, p.batting_average as bat_avg, t.country_name
        FROM PLAYER p JOIN TEAM t ON t.team_id=p.team_id
        WHERE p.no_of_totalruns > 100 AND p.batting_average > 0 AND p.strike_rate > 0
    """)
    if not batters: return jsonify({"hull": [], "all": []})
    
    players_list = [dict(b) for b in batters]
    hull = daa_algos.convex_hull(players_list)
    
    return jsonify({"hull": hull, "all": players_list})

@app.route('/api/daa/floyd_warshall')
def daa_floyd_warshall():
    teams_db = query("SELECT team_name FROM TEAM")
    if not teams_db: return jsonify({})
    teams = [t['team_name'] for t in teams_db]
    
    matches = query("SELECT team_1_name, team_2_name, winner, loser, win_margin FROM MATCHES")
    
    matrix = daa_algos.floyd_warshall_dominance(teams, matches)
    
    # Format for JSON
    res = []
    for t1 in teams:
        row = {"team": t1, "distances": {}}
        for t2 in teams:
            dist = matrix[t1][t2]
            row["distances"][t2] = dist if dist != float('inf') else -1
        res.append(row)
        
    return jsonify({"teams": teams, "matrix": res})

@app.route('/api/daa/huffman')
def daa_huffman():
    # Gather frequencies of dismissals from player_match_stats
    stats = query("SELECT dismissal, COUNT(*) as count FROM PLAYER_MATCH_STATS WHERE dismissal != '' AND dismissal IS NOT NULL GROUP BY dismissal")
    
    freqs = {}
    if stats:
        for s in stats:
            d = s['dismissal'].lower()
            if 'bowled' in d: freqs['bowled'] = freqs.get('bowled',0) + s['count']
            elif 'caught' in d: freqs['caught'] = freqs.get('caught',0) + s['count']
            elif 'lbw' in d: freqs['lbw'] = freqs.get('lbw',0) + s['count']
            elif 'run out' in d: freqs['run_out'] = freqs.get('run_out',0) + s['count']
            elif 'stumped' in d: freqs['stumped'] = freqs.get('stumped',0) + s['count']
            else: freqs['other'] = freqs.get('other',0) + s['count']
    
    # Ensure minimum data for visualization
    if not freqs or len(freqs) < 2:
        freqs = {"caught": 150, "bowled": 60, "lbw": 40, "run_out": 25, "stumped": 10, "other": 5}
        
    codes, tree = daa_algos.huffman_encoding(freqs)
    return jsonify({"frequencies": freqs, "codes": codes, "tree": tree})

@app.route('/api/daa/greedy_fantasy', methods=['POST'])
def daa_greedy_fantasy():
    d = request.json or {}
    total_budget = float(d.get('budget', 100.0))
    all_p = get_fantasy_players()
    
    available_bat = [p for p in all_p if p['role'] == 'Batter']
    available_bowl = [p for p in all_p if p['role'] == 'Bowler']
    available_wkar = [p for p in all_p if p['role'] == 'WKAR']
    
    import random
    for p in available_bat + available_bowl + available_wkar:
        p['value'] = round(p['value'] + random.uniform(-2.0, 2.0), 2)
    
    res = daa_algos.greedy_fantasy(available_bat, available_bowl, available_wkar, 5, 5, 1, total_budget)
    
    return jsonify({
        "suggested": res["team"],
        "max_value": res["max_value"],
        "rem_budget": total_budget - sum(p['cost'] for p in res["team"]),
        "updated_pool": available_bat + available_bowl + available_wkar
    })

@app.route('/api/daa/kmeans')
def daa_kmeans():
    batters = query("""
        SELECT p.player_name, p.strike_rate as sr, p.batting_average as bat_avg, t.country_name
        FROM PLAYER p JOIN TEAM t ON t.team_id=p.team_id
        WHERE p.no_of_totalruns >= 100 AND p.strike_rate > 0 AND p.batting_average > 0
    """)
    if not batters: return jsonify([])
    
    players_list = [dict(b) for b in batters]
    clustered = daa_algos.k_means_clustering(players_list, k=3)
